...
class subkernel:
    ...
    def __init__(
        ...
        language: str = "",
        color: str = "",
        options: Optional[Dict[str, Union[str, Dict]]] = None,
        codemirror_mode: str = "",
    ) -> None:
        if options is None:
            options = {}
...
class Subkernels:
    ...
    def __init__(self, kernel) -> None:
        ...
        for x in self.language_info.keys():
            for lname, knames in kernel.supported_languages[
                x
            ].supported_kernels.items():
                for kname in knames:
                    if x != kname:
                        lan_map[kname] = (
                            lname,
                            self.get_background_color(self.language_info[x], lname),
    ...
    def kernel_list(self) -> List[subkernel]:
    ...
    def add_or_replace(self, kdef: subkernel) -> subkernel:
    ...
    def get_background_color(self, plugin, lan: str) -> str:
    ...
    def find(
        ...
        codemirror_mode = (
            codemirror_mode
            if codemirror_mode
            else self.default_cm_mode.get("codemirror_mode", codemirror_mode)
        )
            ...
            if ":" in language:
                ...
                try:
                    ...
                    for (
                        supported_lan,
                        supported_kernels,
                    ) in plugin.supported_kernels.items():
                        for supported_kernel in supported_kernels:
                            if (
                                name != supported_kernel
                                and supported_kernel not in self.language_info
                            ):
        ...
        if language is not None:
            ...
            if ":" in language:
                ...
                avail_kernels = [
                    ...
                    if y.kernel in sum(plugin.supported_kernels.values(), [])
                    or any(
                ...
                if not avail_kernels:
                    raise ValueError(
                        'Failed to find any of the kernels {} supported by language {}. Please make sure it is properly installed and appear in the output of command "jupyter kenelspec list"'.format(
                            ", ".join(sum(plugin.supported_kernels.values(), [])),
                            language,
                        )
                    )
                ...
                lan_name = list(
                    {
                        ...
                        if avail_kernels[0] in y
                        or any(fnmatch.fnmatch(avail_kernels[0], z) for z in y)
                    }.keys()
                )[0]
                if color == "default":
    ...
    def update(self, notebook_kernel_list: List[List[str]]) -> None:
