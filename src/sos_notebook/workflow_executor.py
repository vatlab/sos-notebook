...
from sos.utils import TerminateExecution, _parse_error, env, get_traceback, pexpect_run
    ...
    def emit(self, record: logging.LogRecord) -> None:
        ...
        self.kernel.send_frontend_msg(
            "display_data",
            {
                "metadata": {},
                "data": {
                    "text/html": f'<div class="sos_logging sos_{record.levelname.lower()}">{record.levelname}: {msg}</div>'
                },
            },
        )
...
def execute_scratch_cell(
    ...
    config = {
        ...
        "worker_procs": args.__worker_procs__
        if hasattr(args, "__worker_procs__")
        else args.__max_procs__,
    ...
    def run(self) -> None:
        ...
        try:
            ...
            ret_code = pexpect_run(
                subprocess.list2cmdline(cmd), shell=True, stdout_socket=stdout_socket
            )
...
def cancel_workflow(cell_id: str, kernel: Any) -> None:
    ...
    kernel.send_frontend_msg(
        "workflow_status", {"cell_id": cell_id, "status": "purged"}
    )
